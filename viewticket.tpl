...
{foreach from=$statuses item=statusitem}
    <option{if $statusitem.title eq $status} selected{/if} style="color:{$statusitem.color}">
        {if $statusitem.title == 'Open'} Abierto
            {elseif $statusitem.title == 'Answered'} Contestado
            {elseif $statusitem.title == 'Customer-Reply'} Respuesta del Cliente
            {elseif $statusitem.title == 'Closed'} Cerrado
            {else} {$statusitem.title}
        {/if}
    </option>
{/foreach}
